...
import UserWhatsapp from "../models/User";
...
export const initIO = (httpServer: Server): SocketIO => {
  ...
  io.on("connection", async socket => {
    ...
    socket.on(
      "joinConnectionTickets",
      async (whatsappId: string, status: string) => {
        logger.info(
          `User ${user.id} joining connection ${whatsappId} tickets channel with status ${status}`
        );
        ...
        if (user.profile === "admin" || user.profile === "supervisor") {
          if (whatsappId === "all") {
            ...
            socket.join(
              `company-${user.companyId}-whatsapp-${whatsappId}-${status}`
            );
          }
          logger.info(
            `Admin/Supervisor ${user.id} joined whatsapp ${whatsappId} ${status} channel`
          );
        } else {
          socket.join(
            `company-${user.companyId}-whatsapp-${whatsappId}-${status}`
          ...
          logger.info(
            `User ${user.id} joined whatsapp ${whatsappId} ${status} channel`
          );
        }
      }
    );
