...
interface UniversalRequest {
  ticket: Ticket;
  message: any; // Mensagem universal (WhatsApp ou Facebook)
  wbot?: any; // WhatsApp bot (opcional)
  typebot: QueueIntegrations;
  platform: 'whatsapp' | 'facebook' | 'instagram';
}

const typebotListenerUniversal = async ({
  ticket,
  message,
  wbot,
  typebot,
  platform
}: UniversalRequest): Promise<void> => {
  ...
  console.log("🤖 Typebot Universal iniciado:", {
    ...
    body,
    number,
    hasSession: !isNil(ticket.typebotSessionId),
    isFirstMessage: isNil(ticket.typebotSessionId)
  });
  ...
  try {
    ...
    const isNewSession = isNil(ticket.typebotSessionId);
      ...
      if (msg.type === 'text') {
        ...
        if (formattedText.startsWith("#")) {
          ...
          try {
            ...
            if (!isNil(jsonGatilho.queueId) && jsonGatilho.queueId > 0) {
              ticketData.queueId = jsonGatilho.queueId;
            }
            
            if (!isNil(jsonGatilho.userId) && jsonGatilho.userId > 0) {
            ...
            logger.error("Erro ao processar comando JSON:", err);
          }
        }
    ...
    logger.error("Error on typebotListenerUniversal: ", error);
    await ticket.update({
      typebotSessionId: null,
      typebotStatus: false
    });
...
export default typebotListenerUniversal;