...
import ProcessMessageService from "./services/ScheduleServices/ProcessMessageService";
...
async function handleSendScheduledMessage(job) {
  ...
  try {
    const whatsapp = scheduleRecord.whatsappId
      ? await Whatsapp.findByPk(scheduleRecord.whatsappId)
      : await GetDefaultWhatsApp(schedule.companyId);
    ...
    const processedBody = await ProcessMessageService({
      body: schedule.body,
    ...
    await SendMessage(whatsapp, {
      ...
      body: formatBody(processedBody, schedule.contact),
      mediaPath: filePath
    });
    ...
    if (scheduleRecord.queueId || scheduleRecord.userId || scheduleRecord.ticketStatus) {
      ...
      if (ticket) {
        ...
        if (scheduleRecord.queueId) updateData.queueId = scheduleRecord.queueId;
        if (scheduleRecord.userId) updateData.userId = scheduleRecord.userId;
        if (scheduleRecord.ticketStatus) updateData.status = scheduleRecord.ticketStatus;
        ...
        if (Object.keys(updateData).length > 0) {
          ...
          logger.info(`Ticket atualizado após agendamento: ${ticket.id}`);
        }
      }
    }
...
async function processScheduleRecurrence(schedule: Schedule): Promise<void> {
  try {
    ...
    if (schedule.mediaPath && schedule.mediaName) {
      const filesOptions = await FilesOptions.findAll({
        where: {
    ...
    logger.info(`Agendamento recorrente criado: ID=${newSchedule.id}, próximo envio em ${moment(nextDate).format("DD/MM/YYYY HH:mm:ss")}`);
    const io = getIO();
    ...
    Sentry.captureException(e);
    logger.error("Recorrência de agendamento: error", e.message);
  }
}
...
async function handleLoginStatus(job) {
  const users: { id: number }[] = await sequelize.query(
    `select id from "Users" where "updatedAt" < now() - '30 minutes'::interval and online = true`, // Alterado de 5 para 30 minutos
    { type: QueryTypes.SELECT }
  );
