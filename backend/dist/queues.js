...
const ProcessMessageService_1 = __importDefault(require("./services/ScheduleServices/ProcessMessageService"));
...
async function handleSendScheduledMessage(job) {
    ...
    try {
        const whatsapp = scheduleRecord.whatsappId
            ? await Whatsapp_1.default.findByPk(scheduleRecord.whatsappId)
            : await (0, GetDefaultWhatsApp_1.default)(schedule.companyId);
        let filePath = null;
        if (schedule.mediaPath) {
            filePath = path_1.default.resolve("public", schedule.mediaPath);
        }
        const processedBody = await (0, ProcessMessageService_1.default)({
            body: schedule.body,
            ...
            sendSignature: schedule.sendSignature
        });
        await (0, SendMessage_1.SendMessage)(whatsapp, {
            number: schedule.contact.number,
            body: (0, Mustache_1.default)(processedBody, schedule.contact),
            mediaPath: filePath
        });
        ...
        if (scheduleRecord.queueId || scheduleRecord.userId || scheduleRecord.ticketStatus) {
            ...
            if (ticket) {
                const updateData = {};
                if (scheduleRecord.queueId)
                    updateData.queueId = scheduleRecord.queueId;
                if (scheduleRecord.userId)
                    updateData.userId = scheduleRecord.userId;
                if (scheduleRecord.ticketStatus)
                    updateData.status = scheduleRecord.ticketStatus;
                if (Object.keys(updateData).length > 0) {
                    await ticket.update(updateData);
                    const io = (0, socket_1.getIO)();
                    io.to(`company-${scheduleRecord.companyId}-mainchannel`)
                        .to(ticket.status)
                        .to("notification")
                        .emit(`company-${scheduleRecord.companyId}-ticket`, {
                        action: "update",
                        ticket
                    });
                    logger_1.logger.info(`Ticket atualizado após agendamento: ${ticket.id}`);
                }
            }
        }
        //Procesasr recorrência após ter enviado com sucesso 
        if (scheduleRecord && scheduleRecord.recurrence) {
            await processScheduleRecurrence(scheduleRecord);
        }
...
/*
  * Processa a recorrencia de um agendamento após seu envio bem-sucedido
  * @param schedule - O agendamento a ser processado
*/
async function processScheduleRecurrence(schedule) {
    try {
        if (!schedule.recurrence) {
            return;
        }
        if (schedule.recurrenceCount <= 1) {
            return;
        }
        let nextDate = new Date(schedule.sendAt);
        const interval = schedule.recurrenceInterval;
        const value = schedule.recurrenceValue || 1;
        switch (interval) {
            case "dias":
        ...
        if (schedule.recurrenceDayOptions === "business") {
        ...
        else if (schedule.recurrenceDayOptions === "skip") {
            const day = nextDate.getDay();
            if (day === 0 || day === 6) {
                switch (interval) {
                    case "dias":
                        ...
                        break;
                    case "semanas":
                        nextDate.setDate(nextDate.getDate() + (value * 7) + 2);
                        break;
                    case "meses":
                        nextDate.setMonth(nextDate.getMonth() + value + 1);
                        break;
                    default:
                        return;
                }
        ...
        // Cria um novo agendamento com base no atual
        const newScheduleData = {
            ...
            recurrenceValue: schedule.recurrenceValue,
            recurrenceCount: schedule.recurrenceCount - 1,
            recurrenceDayOptions: schedule.recurrenceDayOptions,
            openTicket: schedule.openTicket,
            sendSignature: schedule.sendSignature,
            mediaPath: schedule.mediaPath,
            mediaName: schedule.mediaName
        };
        const newSchedule = await Schedule_1.default.create(newScheduleData);
        if (schedule.mediaPath && schedule.mediaName) {
            const filesOptions = await FilesOptions_1.default.findAll({
                where: {
                    fileListId: schedule.mediaPath
                }
            });
            filesOptions.map(async (file) => {
                await file.update({
                    scheduleId: newSchedule.id
                });
            });
        }
        logger_1.logger.info(`Agendamento recorrente criado: ID=${newSchedule.id}, próximo envio em ${(0, moment_1.default)(nextDate).format("DD/MM/YYYY HH:mm:ss")}`);
        const io = (0, socket_1.getIO)();
        ...
        Sentry.captureException(e);
        logger_1.logger.error("Recorrência de agendamento: error", e.message);
    }
}
...
async function handleLoginStatus(job) {
    const users = await database_1.default.query(`select id from "Users" where "updatedAt" < now() - '30 minutes'::interval and online = true`, // Alterado de 5 para 30 minutos
