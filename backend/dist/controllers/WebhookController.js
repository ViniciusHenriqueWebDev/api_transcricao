...
exports.subscribeInstagramApp = exports.instagramOAuth = exports.instagramWebhook = exports.webHook = exports.index = void 0;
        ...
        if (body.object === "page" || body.object === "instagram") {
            ...
            for (const entry of body.entry) {
                try {
                    ...
                    console.log(`🔍 Found ${whatsappConnections.length} Instagram connections for pageId ${pageId}:`, whatsappConnections.map(w => ({
                        id: w.id,
                        name: w.name,
                        channel: w.channel,
                    ...
                    if (!whatsappConnections || whatsappConnections.length === 0) {
                        ...
                        const allInstagramConnections = await Whatsapp_1.default.findAll({
                            where: {
                                channel: "instagram",
                                status: "CONNECTED"
                            },
...
const instagramOAuth = async (req, res) => {
    try {
      ...
      `);
        ...
        try {
            const tokenResponse = await axios_1.default.post('https://graph.facebook.com/v18.0/oauth/access_token', 
        ...
        `);
            ...
            const igUserResponse = await axios_1.default.get(`https://graph.facebook.com/v18.0/${instagramAccount.id}`, {
                params: {
        ...
        `);
          ...
          `);
                ...
                // ✅ CRIAR NOVA CONEXÃO
                const instagramData = {
                    name: `@${igUserData.username || igUserData.name || 'unknown'}`,
                    companyId: companyId,
                    status: "CONNECTED",
                    channel: "instagram",
                    isDefault: false,
                ...
                const result = await (0, CreateWhatsAppService_1.default)(instagramData);
...
const processInstagramMessagingEventWithCompany = async (messagingEvent, whatsapp) => {
    try {
...
const processInstagramMessageChange = async (changeValue, whatsapp) => {
    try {
...
const subscribeInstagramApp = async (instagramId, accessToken) => {
    try {
        ...
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                subscribed_fields: validFields.join(','),
...
exports.subscribeInstagramApp = subscribeInstagramApp;
// ✅ FUNÇÕES AUXILIARES COM ISOLAMENTO TOTAL POR EMPRESA
async function processMessagingEventWithCompany(messagingEvent, whatsapp, channel, companyId) {
    ...
    try {
        await (0, facebookMessageListener_1.handleMessage)(whatsapp, messagingEvent, channel, companyId // ✅ PASSAR COMPANY_ID EXPLICITAMENTE
        );
...
async function processInstagramChangeWithCompany(change, whatsapp, pageId, companyId) {
    ...
    if (change.field === "messages" && change.value) {
        ...
        try {
            await (0, facebookMessageListener_1.handleMessage)(whatsapp, instagramEvent, "instagram", companyId // ✅ PASSAR COMPANY_ID EXPLICITAMENTE
            );
