...
const store = async (req, res) => {
    const { email, password, name, profile, companyId: bodyCompanyId, queueIds, whatsappId, wpp } = req.body;
...
const show = async (req, res) => {
    ...
    const user = await (0, ShowUserService_1.default)(userId, req.user.id);
    return res.status(200).json(user);
};
exports.show = show;
...
const remove = async (req, res) => {
    ...
    await (0, DeleteUserService_1.default)(userId, req.user.id);
    const io = (0, socket_1.getIO)();
    io.to(`company-${companyId}-mainchannel`).emit(`company-${companyId}-user`, {
        action: "delete",
        userId
