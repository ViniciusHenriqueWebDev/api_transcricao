...
const UpdateEmployeeGoalProgressService = async ({ goalId, employeeId, employeeGoalId, progress, individualCurrent, companyId }) => {
    if (progress < 0 || progress > 100) {
        throw new AppError_1.default("O progresso deve estar entre 0 e 100");
    }
    ...
    try {
        ...
        if (goal.metricType === 'produto') {
            if (!employeeGoalId) {
                throw new AppError_1.default("Para metas do tipo produto, é necessário especificar o employeeGoalId");
            }
            // Buscar a relação específica para este produto
            const employeeGoal = await EmployeeGoal_1.default.findOne({
                where: {
                    id: employeeGoalId,
                    goalId,
                    employeeId
            ...
            if (!employeeGoal) {
                throw new AppError_1.default("Produto não encontrado para este funcionário e meta", 404);
            }
            ...
            const totalCurrent = allEmployeeGoals.reduce((sum, eg) => sum + (eg.individualCurrent || 0), 0);
            ...
            if (!employeeGoal) {
                throw new AppError_1.default("Funcionário não está associado a esta meta", 404);
            }
            ...
            if (goal.dividedGoal) {
                ...
                const highestProgress = await EmployeeGoal_1.default.findOne({
                    where: { goalId },
                    order: [['progress', 'DESC']],
                    limit: 1,
                    transaction
        ...
        throw new AppError_1.default(`Erro ao atualizar progresso: ${error.message}`);
    }
};
exports.default = UpdateEmployeeGoalProgressService;
