...
const SetTicketMessagesAsRead = async (ticket) => {
    ...
    if (!ticket || !ticket.id) {
        logger_1.logger.warn("Ticket inválido enviado para SetTicketMessagesAsRead");
        return;
    }
    ...
    if (diffInSeconds < 3) {
        logger_1.logger.info(`Ticket ${ticket.id} foi atualizado recentemente. Pulando chamada ao WhatsApp.`);
        return;
    }
    ...
    try {
        ...
        const wbot = await (0, GetTicketWbot_1.default)(ticket).catch(err => {
            logger_1.logger.warn(`Não foi possível obter wbot para ticket ${ticket.id}: ${err.message}`);
            return null;
        });
        ...
        if (getJsonMessage.length > 0) {
            try {
                ...
                logger_1.logger.warn(`Erro ao marcar mensagens como lidas no WhatsApp: ${err.message}`);
            }
        }
    }
    catch (err) {
        logger_1.logger.warn(`Não foi possível marcar mensagens como lidas: ${err.message}`);
    }
};
