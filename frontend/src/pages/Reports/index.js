...
const Reports = () => {
  ...
  const formatarDadosTicket = (ticket) => {
    return {
      ...ticket,
      contactName: ticket.contactName || "Cliente não identificado",
      contactNumber: ticket.contactNumber || "",
      userName: ticket.userName || "Não atribuído",
      queueName: ticket.queueName || "Sem fila",
  ...
  const exportarGridParaExcel = async () => {
    ...
    try {
      ...
      const ticketsData = data.tickets.map(ticket => {
        ...
        //tratamento para os tickets vazios 
        const formattedTicket = formatarDadosTicket(ticket);
        ...
        return {
          ...
          Conexão: formattedTicket.whatsappName,
          Contato: formattedTicket.contactName,
          Telefone: formattedTicket.contactNumber,
          Usuário: formattedTicket.userName,
          Fila: formattedTicket.queueName,
          Status: formattedTicket.status,
          ÚltimaMensagem: formattedTicket.lastMessage,
          DataHoraAbertura: formattedTicket.createdAt,
          // HoraAbertura: horaCriacao,
          DataHoraFechamento: formattedTicket.closedAt === null ? "" : ticket.closedAt,
          // HoraFechamento: ticket.closedAt === null ? "" : horaFechamento,
          TempoDeAtendimento: formattedTicket.supportTime,
          nps: formattedTicket.NPS,
        }
      });
              ...
              {tickets.map((ticket) => {
                ...
                return (
                  <TableRow key={ticket.id}>
                    ...
                    <TableCell align="left">{formattedTicket.whatsappName}</TableCell>
                    <TableCell align="left">{formattedTicket.contactName}</TableCell>
                    <TableCell align="left">{formattedTicket.contactNumber}</TableCell>
                    <TableCell align="left">{formattedTicket.userName}</TableCell>
                    <TableCell align="left">{formattedTicket.queueName}</TableCell>
                    <TableCell align="center">{formattedTicket.status}</TableCell>
                    <TableCell align="left">{formattedTicket.lastMessage}</TableCell>
                    <TableCell align="center">{formattedTicket.createdAt}</TableCell>
                    <TableCell align="center">{formattedTicket.closedAt}</TableCell>
                    <TableCell align="center">{formattedTicket.supportTime}</TableCell>
                    <TableCell align="center">{formattedTicket.NPS}</TableCell>
