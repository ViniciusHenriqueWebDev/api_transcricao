...
const Invoices = () => {
  ...
  const handleCheckPayment = async (invoiceId) => {
    try {
      setLoading(true);
      ...
      toast.success(`Status do pagamento: ${data.status || 'Verificado'}`);
      
      // Recarregar a página após verificação
      window.location.reload();
    } catch (err) {
      toast.error(err.response?.data?.message || "Erro ao verificar pagamento");
    } finally {
      setLoading(false);
    }
  };
                    ...
                    {rowStatus(invoice) !== "Pago" ? (
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                        <Button
                          ...
                          onClick={() => handleOpenContactModal({...invoice, paymentMethod: "MERCADOPAGO"})}
                        >
                          PAGAR PIX
                        </Button>
                        <Button
                          ...
                          onClick={() => handleCheckPayment(invoice.id)}
                        >
                          Verificar
                        </Button>
                      </div>
