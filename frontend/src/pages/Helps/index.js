  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (user?.profile === "user") {
        filteredHelps = helps.filter(h => h.title.toLowerCase().includes("user"));
      } else if (user?.profile === "supervisor") {
        filteredHelps = helps.filter(
          h => h.title.toLowerCase().includes("supervisor") || h.title.toLowerCase().includes("user")
        );
      }
      // admin vê tudo

      setRecords(filteredHelps);
    }
    fetchData();
  }, [user]);
