...
const useStyles = makeStyles((theme) => ({
  ...
  messageRow: {
    display: "flex",
    marginBottom: 18, // aumente aqui para mais espaçamento
    marginTop: 6,
  },
  messageRowRight: {
    justifyContent: "flex-end",
...
export default function ChatMessages({
  ...
  function getAvatarClass(profile, classes) {
    if (profile === "admin") return classes.admin;
    if (profile === "supervisor") return classes.supervisor;
    return classes.user;
  }
      ...
      <div onScroll={handleScroll} className={classes.messageList}>
        {Array.isArray(messages) &&
          messages.map((item, key) => {
            ...
            const profile = sender.profile || (isMe ? user.profile : "user");
            const avatarClass = getAvatarClass(profile, classes);
            const avatarContent = sender.profilePicUrl
              ? <Avatar src={sender.profilePicUrl} className={`${classes.avatar} ${avatarClass}`} />
              : <Avatar className={`${classes.avatar} ${avatarClass}`}>{(sender.name || "?")[0]}</Avatar>;
            ...
            return (
              <div
                ...
                className={
                  classes.messageRow +
                  " " +
                  (isMe ? classes.messageRowRight : classes.messageRowLeft)
                }
              >
                {!isMe && avatarContent}
                <div
                  className={
                    classes.bubble + " " + (isMe ? classes.bubbleRight : classes.bubbleLeft)
                  }
                >
                  <Typography variant="subtitle2" style={{ fontWeight: 500, marginBottom: 2 }}>
                    {sender.name}
                  </Typography>
