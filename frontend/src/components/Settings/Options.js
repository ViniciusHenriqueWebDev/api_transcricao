...
export default function Options(props) {
  ...
  const [signatureMessage, setSignatureMessage] = useState("disabled");
  const [loadingSignatureMessage, setLoadingSignatureMessage] = useState(false);
  const [mercadoPagoToken, setMercadoPagoToken] = useState("");
  const [loadingMercadoPagoToken, setLoadingMercadoPagoToken] = useState(false);
  const [mercadoPagoPubKey, setMercadoPagoPubKey] = useState("");
  const [loadingMercadoPagoPubKey, setLoadingMercadoPagoPubKey] = useState(false);
  const [showMercadoPagoToken, setShowMercadoPagoToken] = useState(false);
  const [showMercadoPagoPubKey, setShowMercadoPagoPubKey] = useState(false);
  ...
  useEffect(() => {
    if (Array.isArray(settings) && settings.length) {
      ...
      const signatureMessageConfig = settings.find((s) => s.key === "signatureMessage");
      if (signatureMessageConfig) {
        setSignatureMessage(signatureMessageConfig.value);
      }
      const mercadoPagoToken = settings.find((s) => s.key === "mercadopagoToken");
      if (mercadoPagoToken) {
        setMercadoPagoToken(mercadoPagoToken.value);
      }

      const mercadoPagoPubKey = settings.find((s) => s.key === "mercadopagoPubKey");
      if (mercadoPagoPubKey) {
        setMercadoPagoPubKey(mercadoPagoPubKey.value);
      }
    }
  ...
  async function handleSetSignatureMessage(value) {
    setSignatureMessage(value);
    setLoadingSignatureMessage(true)
    await update({
      key: "signatureMessage",
      value,
    });
    toast.success("Configuração de assinatura atualizada com sucesso.");
    setLoadingSignatureMessage(false);
  }
  ...
  // Adicione após a função handleChangeAsaas

  async function handleChangeMercadoPagoToken(value) {
    setMercadoPagoToken(value);
    setLoadingMercadoPagoToken(true);
    await update({
      key: "mercadopagoToken",
      value,
    });
    toast.success("Token do Mercado Pago atualizado com sucesso.");
    setLoadingMercadoPagoToken(false);
  }

  async function handleChangeMercadoPagoPubKey(value) {
    setMercadoPagoPubKey(value);
    setLoadingMercadoPagoPubKey(true);
    await update({
      key: "mercadopagoPubKey",
      value,
    });
    toast.success("Public Key do Mercado Pago atualizado com sucesso.");
    setLoadingMercadoPagoPubKey(false);
  }
  ...
  return (
    <>
      <Grid spacing={3} container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            ...
            <FormHelperText>
              {loadingUserRating
                ? "Atualizando..."
                : "O ticket será fechado automaticamente após 1 minuto sem resposta"}
            </FormHelperText>
          </FormControl>
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
        ...
        <Grid xs={12} sm={6} md={4} item>
          <FormControl className={classes.selectContainer}>
            <InputLabel id="signatureMessage-label">Permitir assinatura para usuarios</InputLabel>
            <Select
              labelId="signatureMessage-label"             // CORRIGIDO: ID do label específico
              value={signatureMessage}                     // CORRIGIDO: Usando variável correta
              onChange={async (e) => {
                handleSetSignatureMessage(e.target.value);
              }}
            >
              <MenuItem value={"disabled"}>Desabilitado</MenuItem>
              <MenuItem value={"enabled"}>Habilitado</MenuItem>
            ...
            <FormHelperText>
              {loadingSignatureMessage && "Atualizando..."}  // CORRIGIDO: Usando variável correta
            </FormHelperText>
          </FormControl>
        </Grid>
      ...
      {/*-----------------MERCADO PAGO-----------------*/}
      {/*-----------------MERCADO PAGO-----------------*/}
      {props.currentUser && props.currentUser.super && (
        <Grid spacing={3} container style={{ marginBottom: 10 }}>
          ...
          <Grid xs={12} sm={12} md={6} item>
            <FormControl className={classes.selectContainer}>
              <TextField
                ...
                onChange={async (e) => {
                  handleChangeMercadoPagoToken(e.target.value);
                }}
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton
                        ...
                        edge="end"
                      >
                        {showMercadoPagoToken ? <VisibilityOff /> : <Visibility />}
                      </IconButton>
                    </InputAdornment>
                  ),
                }}
              />
              <FormHelperText>
                {loadingMercadoPagoToken && "Atualizando..."}
              </FormHelperText>
            </FormControl>
          </Grid>
          ...
          <Grid xs={12} sm={12} md={6} item>
            <FormControl className={classes.selectContainer}>
              <TextField
                id="mercadopagoPubKey"
                name="mercadopagoPubKey"
                margin="dense"
                label="Public Key do Mercado Pago"
                variant="outlined"
                fullWidth
                type={showMercadoPagoPubKey ? "text" : "password"}
                value={mercadoPagoPubKey || ""}
                onChange={async (e) => {
                  handleChangeMercadoPagoPubKey(e.target.value);
                }}
                InputProps={{
                  endAdornment: (
                    <InputAdornment position="end">
                      <IconButton
                        ...
                        onClick={() => setShowMercadoPagoPubKey(!showMercadoPagoPubKey)}
                        edge="end"
                      >
                        {showMercadoPagoPubKey ? <VisibilityOff /> : <Visibility />}
                      </IconButton>
                    </InputAdornment>
                  ),
                }}
              />
              <FormHelperText>
                {loadingMercadoPagoPubKey && "Atualizando..."}
              </FormHelperText>
            </FormControl>
          </Grid>
        </Grid>
