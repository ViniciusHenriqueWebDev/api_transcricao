...
const MessagesList = ({ ticket, ticketId, isGroup, user }) => {
  ...
  useEffect(() => {
    if (user.id) {
      ...
      // ✅ LISTENER PARA MENSAGENS (incluindo Facebook/Instagram)
      socket.on(`company-${companyId}-appMessage`, (data) => {
        console.log("💬 Mensagem recebida no MessagesList:", {
          action: data.action,
          ticketId: data.ticket?.id,
          messageTicketId: data.message?.ticketId,
          currentTicketId: currentTicketId.current,
          channel: data.ticket?.whatsapp?.channel
        });

        // ✅ SÓ PROCESSAR SE FOR PARA O TICKET ATUAL
        if (data.message && data.message.ticketId === currentTicketId.current) {
          if (user.profile === "user" && data.ticket && data.ticket.userId != user.id) {
            return;
          }
          if (data.action === "create" || data.action === "received") {
          ...
          if (data.action === "update") {
            console.log("🔄 Atualizando mensagem:", data.message.id);
