...
const useStyles = makeStyles((theme) => ({
  ...
  connectionIcon: {
    fontSize: "14px", // ✅ Diminui um pouco para caber melhor na bolinha
    marginRight: "0px", // ✅ Remove margin quando na bolinha
  ...
  defaultIcon: {
    color: "#757575", // Cinza para outros canais
  },
  connectionBadge: {
    display: "flex",
    alignItems: "center",
...
const TicketListItemCustom = ({ ticket, updateTicketList, onRedirectToTab, removeTicketFromList }) => {
  ...
  const getConnectionIcon = (channel, whatsappName) => {
    const baseClass = `${classes.connectionIcon}`;
    ...
    switch (channel?.toLowerCase()) {
      ...
      case 'baileys':
        return (
          <WhatsAppIcon
            className={`${baseClass} ${classes.whatsappIcon}`}
      ...
      case 'facebook':
        return (
          <FacebookIcon
            className={`${baseClass} ${classes.facebookIcon}`}
      ...
      case 'instagram':
        return (
          <InstagramIcon
            className={`${baseClass} ${classes.instagramIcon}`}
            titleAccess="Instagram"
          />
        );
      default:
        // Fallback: se não tiver channel mas tiver whatsapp, assumir WhatsApp
        if (whatsappName) {
          return (
            <PhoneAndroidIcon
              className={`${baseClass} ${classes.defaultIcon}`}
              titleAccess="Conexão"
            />
          );
        }
  ...
  // ✅ ADICIONAR ESTA FUNÇÃO PARA PEGAR A COR DO BADGE
  const getConnectionBadgeColor = (channel) => {
    switch (channel?.toLowerCase()) {
      case 'whatsapp':
      case 'baileys':
  ...
  const handleCloseTicket = async (id) => {
    ...
    try {
      ...
      if (removeTicketFromList) removeTicketFromList(id); // Remover imediatamente
      if (updateTicketList) updateTicketList();
    } catch (err) {
      setLoading(false);
      toastError(err);
    }
  ...
  const handleReopenTicket = async (id) => {
    ...
    try {
      ...
      if (removeTicketFromList) removeTicketFromList(id); // Remove imediatamente da lista atual
      if (updateTicketList) updateTicketList();
    } catch (err) {
      setLoading(false);
      toastError(err);
      ...
      >
        ...
        <ListItemAvatar>
          <div style={{ position: "relative" }}>
            ...
            {/* ✅ ÍCONE DA CONEXÃO CENTRALIZADO NA BOLINHA */}
            <div style={{
              position: "absolute",
              bottom: "-4px",
              right: "-4px",
              ...
              {getConnectionIcon(ticket?.whatsapp?.channel || ticket?.channel, ticket?.whatsapp?.name)}
            </div>
          </div>
        </ListItemAvatar>
                ...
                <span style={{ marginTop: 4, }} className={classes.secondaryContentSecond} >
                  {ticket?.whatsapp?.name ? (
                    <Badge
                      ...
                      style={{
                        backgroundColor: getConnectionBadgeColor(ticket?.whatsapp?.channel || ticket?.channel),
                        display: "flex",
                        alignItems: "center"
                      }}
                    >
          ...
          <Box sx={{ display: 'flex' }}>
            {ticket.status === "pending" && (
              <>
                ...
                {/* Adicione o botão de transferir aqui */}
