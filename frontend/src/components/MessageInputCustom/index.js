...
const CustomInput = (props) => {
  ...
  const lastCorrectedWord = useRef("");
  const lastCorrectedSentence = useRef("");

  const { user } = useContext(AuthContext);
  const { list: listQuickMessages } = useQuickMessages();
  const correctionCache = useRef(new Map());

  const correctWord = async (word) => {
    if (correctionCache.current.has(word)) {
      return correctionCache.current.get(word);
    }

    try {
      const response = await axios.post(
        'https://api.languagetool.org/v2/check',
        `text=${encodeURIComponent(word)}&language=pt-BR`,
        {
      ...
      const corrected = response.data.matches?.[0]?.replacements?.[0]?.value || word;
      if (correctionCache.current.size > 100) {
        const firstKey = correctionCache.current.keys().next().value;
        correctionCache.current.delete(firstKey);
      }
      correctionCache.current.set(word, corrected);
  ...
  const correctSentence = async (sentence) => {
    try {
      const response = await axios.post(
        'https://api.languagetool.org/v2/check',
        `text=${encodeURIComponent(sentence)}&language=pt-BR`,
        {
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          timeout: 5000
        }
      ...
      console.log('API indisponível para frase');
    }
    return sentence;
  };

  const checkFullSentence = useCallback(
    debounce(async (text) => {
      if (!autoCorrectEnabled) return;

      setIsChecking(true); // Mostra indicador

      const trimmedText = text.trim();

      if (trimmedText.length >= 10 && trimmedText.split(' ').length >= 2) {
        if (trimmedText !== lastCorrectedSentence.current) {
          lastCorrectedSentence.current = trimmedText;

          const corrected = await correctSentence(trimmedText);

          if (corrected !== trimmedText) {
            const trailingSpaces = text.length - text.trimEnd().length;
            setInputMessage(corrected + ' '.repeat(trailingSpaces));
          }
        }
      }

      setIsChecking(false); // Remove indicador
    }, 2000),
    [autoCorrectEnabled]
  );
  ...
  const handleTextChange = async (newText) => {
    ...
    if (!autoCorrectEnabled || editingMessage) return;
    checkFullSentence.cancel();
    if (newText.endsWith(' ')) {
      const match = newText.trim().match(/([\wáéíóúâêôãõçüñ'-]+)([!?.…,:;]*)$/i);
      if (match) {
        const lastWord = match[1];
        const punctuation = match[2] || '';
        if (lastWord && lastWord.length >= 3 && lastWord !== lastCorrectedWord.current) {
          lastCorrectedWord.current = lastWord;
          const corrected = await correctWord(lastWord);
          if (corrected !== lastWord) {
            // Substitui só a última ocorrência da palavra + pontuação antes do espaço
            const regex = new RegExp(`${lastWord}${punctuation}\\s$`);
            const correctedText = newText.replace(regex, `${corrected}${punctuation} `);
            setInputMessage(correctedText);
            newText = correctedText;
            lastCorrectedSentence.current = "";
          }
        }
      }
    }
    checkFullSentence(newText);
  };
    ...
    <div className={classes.messageInputWrapper} style={{ position: 'relative' }}>
      ...
      <div
        style={{
          ...
          right: "5px",
          fontSize: "10px",
          color: isChecking ? "#FF9800" : (autoCorrectEnabled ? "#4CAF50" : "#999"),
          backgroundColor: isChecking ? "#FFF3E0" : (autoCorrectEnabled ? "#e8f5e9" : "#f5f5f5"),
          padding: "3px 8px",
          borderRadius: "10px",
          cursor: "pointer",
          zIndex: 1000,
        ...
        title={
          isChecking
            ? "Verificando..."
            : (autoCorrectEnabled ? "Clique para desativar correção" : "Clique para ativar correção")
        }
      >
        {isChecking && '🔄 Verificando...'}
        {!isChecking && autoCorrectEnabled && '✓ Correção'}
        {!isChecking && !autoCorrectEnabled && '✗ Sem correção'}
      </div>
      <Autocomplete
        freeSolo
        open={popupOpen}
...
const MessageInputCustom = (props) => {
  ...
  const { signatureEnabled, signMessage, setSignMessage, applySignature } = useSignature();
  ...
  const handleSendMessage = async () => {
    ...
    const message = {
      ...
      fromMe: true,
      mediaUrl: "",
      body: applySignature(inputMessage.trim(), user?.name, editingMessage !== null),
